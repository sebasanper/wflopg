...
def site(parcels):
    ...
    def process_exclave(exclave, layout, scrutinize, enclave):
        ...
        if 'constraints' in exclave:
            ...
            # turbines are inside the exclave if none of the constraints
            # are satisfied
            scrutinize_candidate = scrutinize & ~satisfies
            inside = scrutinize_candidate.any(dim='constraint')
            steps = xr.where(
                scrutinize_candidate,
                distance * (1+1e-9) * exclave['border_seeker'],
                    # 1+1e-9 to avoid round-off ‘outsides’ TODO: more elegantly
                [0, 0]
            ).rename(constraint='candidate')
            scrutinize_candidate = scrutinize_candidate.rename(
                constraint='candidate')
            distance = distance.rename(constraint='candidate')
            if enclave is None:
                # TODO: This case should not really happen, but it does because
                #       we have not yet dealt with circle enclaves here (and
                #       also the first algorithm step starts with an exclave)
                closest = distance.argmin(dim='candidate')
                        # NOTE: argmin decides ties by picking first of minima
                step = xr.where(
                    scrutinize & inside, steps.isel(candidate=closest), [0, 0])
            else:
                # check if correction lies in the encompassing enclave and from
                # those pick the closest one
                candidate_distance, candidate_satisfies = _constraint_common(
                    enclave, layout + steps, scrutinize_candidate)
                good_candidate = (scrutinize_candidate
                                  & candidate_satisfies.all(dim='constraint'))
                distance = distance.where(good_candidate, np.inf)
                closest = distance.argmin(dim='candidate')
                    # NOTE: argmin decides ties by picking first of minima
                step = xr.where(
                    scrutinize, steps.isel(candidate=closest), [0, 0])
                # check whether there is an exclave vertex that is closer
                vertices = exclave['vertices'][~exclave['violates']]
                vertex_distance = xr.where(
                    scrutinize & inside,
                    np.sqrt(np.square(layout - vertices).sum(dim='xy')),
                    np.inf
                )
                closest_vertex = vertex_distance.argmin(dim='vertex')
                vertex_step = xr.where(
                    scrutinize & inside,
                    vertices.isel(vertex=closest_vertex) - layout,
                    [0, 0]
                )
                # choose between c(onstraint) or v(ertex) step, depending on
                # which is closer
                cv_distance = xr.concat(
                    [distance.isel(candidate=closest),
                     vertex_distance.isel(vertex=closest_vertex)],
                    'cv'
                )
                cv_step = xr.concat([step, vertex_step], 'cv')
                step = cv_step.isel(cv=cv_distance.argmin(dim='cv'))
                enclave = None
